#ifndef PHOENIX_COMMAND_HANDLER_HPP
#define PHOENIX_COMMAND_HANDLER_HPP
...
struct phoenix_command_handler {
    using printer_t = expr_printer<var_names, functor_names>;
    using SolveTimer     = solve_timer<steady_now>;
    using PrintBindings  = print_bindings<phoenix_runtime, printer_t>;
    using PrintProgress  = print_progress<phoenix_runtime, SolveTimer>;
    using PausePoller    = pause_poller<PrintProgress, stdin_is_tty, stdin_try_read_byte, stdin_idle>;
    using SolveLoop      = solve_loop<phoenix_runtime, printer_t, PrintBindings, PrintProgress,
...
private:
    ...
    std::optional<phoenix_runtime> runtime_;
    steady_now clock_;
