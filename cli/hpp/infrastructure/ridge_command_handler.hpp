...
struct ridge_command_handler {
    ...
    using PrintBindings  = print_bindings<ridge_runtime, printer_t>;
    using PrintProgress  = print_progress<ridge_runtime, SolveTimer>;
    using PausePoller    = pause_poller<PrintProgress, stdin_is_tty, stdin_try_read_byte, stdin_idle>;
    using SolveLoop      = solve_loop<ridge_runtime, printer_t, PrintBindings, PrintProgress,
