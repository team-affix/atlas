#ifndef PHOENIX_FC_COMMAND_HANDLER_HPP
#define PHOENIX_FC_COMMAND_HANDLER_HPP
...
struct phoenix_fc_command_handler {
    ...
    using PrintBindings  = print_bindings<phoenix_fc_runtime, printer_t>;
    using PrintProgress  = print_progress<phoenix_fc_runtime, SolveTimer>;
    using PausePoller    = pause_poller<PrintProgress, stdin_is_tty, stdin_try_read_byte, stdin_idle>;
    using SolveLoop      = solve_loop<phoenix_fc_runtime, printer_t, PrintBindings, PrintProgress,
...
private:
    ...
    std::optional<phoenix_fc_runtime> runtime_;
    steady_now clock_;
