#ifndef DBUCT_GENIUS_FC_COMMAND_HANDLER_HPP
#define DBUCT_GENIUS_FC_COMMAND_HANDLER_HPP
...
// CLI handler for the `dbuct-genius-fc` command: camping DBUCT + genius dual reward + FC.
struct dbuct_genius_fc_command_handler {
    using SolveTimer     = solve_timer<steady_now>;
    using PrintBindings  = print_bindings<dbuct_genius_fc_runtime, expr_printer>;
    using BasePP         = print_progress<dbuct_genius_fc_runtime, SolveTimer>;
    using PrintProgress  = genius_print_progress<BasePP, dbuct_genius_fc_runtime>;
    using SolveLoop      = solve_loop<dbuct_genius_fc_runtime, expr_printer, PrintBindings, PrintProgress,
                                      SolveTimer, SolveTimer>;

    dbuct_genius_fc_command_handler(
...
private:
    ...
    std::optional<dbuct_genius_fc_runtime> runtime_;
    steady_now clock_;
