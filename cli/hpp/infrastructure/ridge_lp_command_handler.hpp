#ifndef RIDGE_LP_COMMAND_HANDLER_HPP
#define RIDGE_LP_COMMAND_HANDLER_HPP
...
struct ridge_lp_command_handler {
    using SolveTimer     = solve_timer<steady_now>;
    using PrintBindings  = print_bindings<ridge_lp_runtime, expr_printer>;
    using PrintProgress  = print_progress<ridge_lp_runtime, SolveTimer>;
    using SolveLoop      = solve_loop<ridge_lp_runtime, expr_printer, PrintBindings, PrintProgress,
                                      SolveTimer, SolveTimer>;

    ridge_lp_command_handler(
...
private:
    ...
    std::optional<ridge_lp_runtime> runtime_;
    steady_now clock_;
