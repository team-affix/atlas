#ifndef DBUCT_QUELL_FC_COMMAND_HANDLER_HPP
#define DBUCT_QUELL_FC_COMMAND_HANDLER_HPP
...
// CLI handler for the `dbuct-quell-fc` command: parses the database and goals, wires
// up a dbuct_quell_fc_runtime (camping DBUCT + quell remaining-work reward + fewer-candidate
// rollout), and drives the shared solve loop that prints solutions and progress.
struct dbuct_quell_fc_command_handler {
    ...
    using PrintBindings  = print_bindings<dbuct_quell_fc_runtime, printer_t>;
    ...
    using SolveLoop      = solve_loop<dbuct_quell_fc_runtime, printer_t, PrintBindings, PrintProgress,
...
private:
    ...
    std::optional<dbuct_quell_fc_runtime> runtime_;
    steady_now clock_;
