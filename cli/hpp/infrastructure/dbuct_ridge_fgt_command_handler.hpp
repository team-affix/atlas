#ifndef DBUCT_RIDGE_FGT_COMMAND_HANDLER_HPP
#define DBUCT_RIDGE_FGT_COMMAND_HANDLER_HPP
...
struct dbuct_ridge_fgt_command_handler {
    using SolveTimer     = solve_timer<steady_now>;
    using PrintBindings  = print_bindings<dbuct_ridge_fgt_runtime, expr_printer>;
    using PrintProgress  = print_progress<dbuct_ridge_fgt_runtime, SolveTimer>;
    using PausePoller    = pause_poller<PrintProgress, stdin_is_tty, stdin_try_read_byte, stdin_idle>;
    using SolveLoop      = solve_loop<dbuct_ridge_fgt_runtime, expr_printer, PrintBindings, PrintProgress,
...
private:
    ...
    std::optional<dbuct_ridge_fgt_runtime> runtime_;
    steady_now clock_;
