#ifndef DBUCT_PHOENIX_COMMAND_HANDLER_HPP
#define DBUCT_PHOENIX_COMMAND_HANDLER_HPP
...
// CLI handler for the `dbuct-phoenix` command: parses the database and goals, wires
// up a dbuct_phoenix_runtime (the delayed-backtracking solver), and drives the shared
// solve loop that prints solutions and progress.
struct dbuct_phoenix_command_handler {
    ...
    using PrintBindings  = print_bindings<dbuct_phoenix_runtime, printer_t>;
    using PrintProgress  = print_progress<dbuct_phoenix_runtime, SolveTimer>;
    using PausePoller    = pause_poller<PrintProgress, stdin_is_tty, stdin_try_read_byte, stdin_idle>;
    using SolveLoop      = solve_loop<dbuct_phoenix_runtime, printer_t, PrintBindings, PrintProgress,
...
private:
    ...
    std::optional<dbuct_phoenix_runtime> runtime_;
    steady_now clock_;
