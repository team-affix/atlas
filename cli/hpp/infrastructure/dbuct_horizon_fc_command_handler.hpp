#ifndef DBUCT_HORIZON_FC_COMMAND_HANDLER_HPP
#define DBUCT_HORIZON_FC_COMMAND_HANDLER_HPP
...
// CLI handler for the `dbuct-horizon-fc` command: parses the database and goals, wires
// up a dbuct_horizon_fc_runtime (camping DBUCT + horizon reward + fewer-candidate rollout),
// and drives the shared solve loop that prints solutions and progress.
struct dbuct_horizon_fc_command_handler {
    using SolveTimer     = solve_timer<steady_now>;
    using PrintBindings  = print_bindings<dbuct_horizon_fc_runtime, expr_printer>;
    using BasePP         = print_progress<dbuct_horizon_fc_runtime, SolveTimer>;
    using PrintProgress  = horizon_print_progress<BasePP, dbuct_horizon_fc_runtime>;
    using SolveLoop      = solve_loop<dbuct_horizon_fc_runtime, expr_printer, PrintBindings, PrintProgress,
...
private:
    ...
    std::optional<dbuct_horizon_fc_runtime> runtime_;
    steady_now clock_;
