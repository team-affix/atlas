...
ridge_lp_runtime::ridge_lp_runtime(
    db& database,
...
bool ridge_lp_runtime::next() {
    return manifest_.driver_.next();
}

bool ridge_lp_runtime::solved() const {
    return manifest_.driver_.solved();
}

const expr* ridge_lp_runtime::normalize(framed_expr fe) {
    return manifest_.normalizer_.normalize(fe);
}

size_t ridge_lp_runtime::resolution_depth() const {
    return manifest_.resolution_memory_.get_resolution_count();
}

size_t ridge_lp_runtime::decision_depth() const {
    return manifest_.decision_memory_.count();
}

lemma ridge_lp_runtime::derive_decision_lemma() const {
    return manifest_.decision_memory_.derive_decision_lemma();
}

lemma ridge_lp_runtime::derive_resolution_lemma() const {
