...
dbuct_genius_fc_runtime::dbuct_genius_fc_runtime(
    ...
    : manifest_(database, goals, initial_frame_offset, max_resolutions,
                random_seed, ridge_exploration_constant, horizon_exploration_constant,
                grant_increment_interval) {}

bool dbuct_genius_fc_runtime::next() {
    return manifest_.driver_.next();
}

bool dbuct_genius_fc_runtime::solved() const {
...
size_t dbuct_genius_fc_runtime::decision_depth() const {
    return manifest_.decision_memory_.count();
}

double dbuct_genius_fc_runtime::cgw() const {
    return manifest_.cumulative_grounded_weight_.get();
}

lemma dbuct_genius_fc_runtime::derive_decision_lemma() const {
    return manifest_.decision_memory_.derive_decision_lemma();
}

lemma dbuct_genius_fc_runtime::derive_resolution_lemma() const {
