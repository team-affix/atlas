...
TEST_F(NormalizerUnitTest, FunctorRebuildsViaPool) {
    EXPECT_CALL(pool, make_functor(functors.id("f"), ElementsAre(&arg_whnf0)))
...
TEST_F(NormalizerUnitTest, TernaryFunctorNormalizesAllArgs) {
    ...
    EXPECT_CALL(pool, make_functor(functors.id("f"), ElementsAre(&arg_whnf0, &arg_whnf1, &arg_whnf2)))
        .WillOnce(Return(&pooled_f));
