// dbuct_horizon_terminate_sim: snapshots horizon reward then delegates to dbuct_sim.
...
using test_dbuct_horizon_terminate_sim_t = dbuct_horizon_terminate_sim<
    MockComputeMctsReward, MockSetValueDelta, MockTerminateDbuct>;

struct DbuctHorizonTerminateSimTest : public ::testing::Test {
    MockComputeMctsReward compute_mcts_reward;
    MockSetValueDelta set_value_delta;
    MockTerminateDbuct terminate_dbuct;
    test_dbuct_horizon_terminate_sim_t terminate{
        compute_mcts_reward, set_value_delta, terminate_dbuct};
};

TEST_F(DbuctHorizonTerminateSimTest, SetsRewardThenDelegatesAndForwardsEliminations) {
