...
TEST_F(ExprPoolTest, NullaryFunctorInternedTwiceReturnsSamePointer) {
    EXPECT_EQ(pool.make_functor(atoms.id("f"), {}), pool.make_functor(atoms.id("f"), {}));
}

TEST_F(ExprPoolTest, DifferentFunctorsReturnDifferentPointers) {
    EXPECT_NE(pool.make_functor(atoms.id("f"), {}), pool.make_functor(atoms.id("g"), {}));
}

TEST_F(ExprPoolTest, FunctorWithVarArgInternedTwiceReturnsSamePointer) {
    const expr* v = pool.make_var(0);
    EXPECT_EQ(pool.make_functor(atoms.id("f"), {v}), pool.make_functor(atoms.id("f"), {v}));
}

TEST_F(ExprPoolTest, FunctorsSameNameDifferentArityReturnDifferentPointers) {
    const expr* v = pool.make_var(0);
    EXPECT_NE(pool.make_functor(atoms.id("f"), {}), pool.make_functor(atoms.id("f"), {v}));
}
...
TEST_F(ExprPoolTest, BinaryFunctorInternedTwiceReturnsSamePointer) {
    ...
    EXPECT_EQ(pool.make_functor(atoms.id("f"), {v0, v1}), pool.make_functor(atoms.id("f"), {v0, v1}));
}

TEST_F(ExprPoolTest, FunctorsWithDifferentArgsReturnDifferentPointers) {
    const expr* v0 = pool.make_var(0);
    const expr* v1 = pool.make_var(1);
    EXPECT_NE(pool.make_functor(atoms.id("f"), {v0, v1}), pool.make_functor(atoms.id("f"), {v1, v1}));
}
...
TEST_F(ExprPoolTest, TernaryFunctorInternedTwiceReturnsSamePointer) {
    ...
    EXPECT_EQ(pool.make_functor(atoms.id("h"), {v0, v1, v2}), pool.make_functor(atoms.id("h"), {v0, v1, v2}));
}
...
TEST_F(ExprPoolTest, ImportTernaryFunctorInternsEachArg) {
    ...
    expr b{expr::var{1}};
    expr c{expr::var{2}};
    expr root{expr::functor{atoms.id("h"), {&a, &b, &c}}};
...
TEST_F(ExprPoolTest, ImportNullaryAtomDedupesWithMakeFunctor) {
    expr e{expr::functor{atoms.id("abc"), {}}};
    const expr* pooled = pool.make_functor(atoms.id("abc"), {});
    EXPECT_EQ(pool.import(&e), pooled);
}

TEST_F(ExprPoolTest, ImportFunctorWithAtomArgs) {
    expr abc{expr::functor{atoms.id("abc"), {}}};
    expr num{expr::functor{atoms.id("123"), {}}};
    expr root{expr::functor{atoms.id("f"), {&abc, &num}}};

    const expr* p = pool.import(&root);
    const expr::functor& f = std::get<expr::functor>(p->content);
    ASSERT_EQ(f.args.size(), 2u);
    EXPECT_EQ(f.args[0], pool.make_functor(atoms.id("abc"), {}));
    EXPECT_EQ(f.args[1], pool.make_functor(atoms.id("123"), {}));
}
