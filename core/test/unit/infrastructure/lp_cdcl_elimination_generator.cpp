...
TEST_F(LpCdclEliminationGeneratorTest, UnitOnDescentIsEmittedAndNotRecachedAsLive) {
    ...
    EXPECT_THAT(decide(&g0_r0), ElementsAre(&g1_r1));
    // A different first decision from the root does not fire the survivor.
    cdcl.cleanup();
    enter_root();
    EXPECT_THAT(decide(&g2_r2), IsEmpty());
}

TEST_F(LpCdclEliminationGeneratorTest, RevisitOfUnchangedPrefixKeepsTheCachedReduction) {
    learn_over({&g0_r0, &g1_r1, &g2_r2});
    EXPECT_THAT(decide(&g0_r0), IsEmpty());
    cdcl.cleanup();
    enter_root();
...
TEST_F(LpCdclEliminationGeneratorTest, ParentUpdateReachesAGrandchildOnTheNextVisit) {
    learn_over({&g0_r0, &g1_r1, &g2_r2, &g3_r3});
    EXPECT_THAT(decide(&g0_r0), IsEmpty());
    EXPECT_THAT(decide(&g1_r1), IsEmpty());
    cdcl.cleanup();
...
TEST_F(LpCdclEliminationGeneratorTest, SatisfiedAvoidanceStaysDeadOnALaterVisit) {
    learn_over({&g0_r0, &g1_r1, &g2_r2});
    EXPECT_THAT(unit(&g0_r9), IsEmpty());
    EXPECT_THAT(decide(&g1_r1), IsEmpty());
    cdcl.cleanup();
...
TEST_F(LpCdclEliminationGeneratorTest, CleanupDropsUnflushedEliminations) {
    cdcl.descend(&g0_r0);
    cdcl.learn();
    cdcl.cleanup();
    enter_root();
...
TEST_F(LpCdclEliminationGeneratorTest, LearnIsAppliedAtTheNextRootEntry) {
    cdcl.descend(&g0_r0);
    cdcl.descend(&g1_r1);
    cdcl.learn();
    cdcl.enter();
...
TEST_F(LpCdclEliminationGeneratorTest, SameDecisionSetKeepsSatisfactionAcrossArrivalOrders) {
    learn_over({&g0_r0, &g1_r1, &g2_r2, &g3_r3});
    EXPECT_THAT(decide(&g0_r0), IsEmpty());
    EXPECT_THAT(decide(&g1_r1), IsEmpty());
    EXPECT_THAT(unit(&g2_r8), IsEmpty());
    cdcl.cleanup();
    enter_root();
    EXPECT_THAT(decide(&g1_r1), IsEmpty());
    EXPECT_THAT(decide(&g0_r0), IsEmpty());
...
TEST_F(LpCdclEliminationGeneratorTest, ForcedEliminationIsVisibleOnADifferentChildEntry) {
    learn_over({&g0_r0, &g1_r1, &g2_r2});
    EXPECT_THAT(unit(&g0_r0), IsEmpty());
    EXPECT_THAT(unit(&g1_r1), ElementsAre(&g2_r2));
    cdcl.descend(&g4_r4);
