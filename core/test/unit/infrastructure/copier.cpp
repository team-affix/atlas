...
protected:
    void SetUp() override {
        ...
        ON_CALL(pool, make_var(0)).WillByDefault(Return(&pooled_v0));
        ON_CALL(pool, make_var(1)).WillByDefault(Return(&pooled_v1));
        ON_CALL(pool, make_var(2)).WillByDefault(Return(&pooled_v2));
    }
    ...
    expr pooled_f{expr::functor{0, {}}};
    expr pooled_g{expr::functor{0, {}}};
};

TEST_F(CopierUnitTest, FirstVarAllocatesFreshIndex) {
    EXPECT_CALL(seq, next()).WillOnce(Return(7u));
    EXPECT_CALL(pool, make_var(7u)).WillOnce(Return(&pooled_v7));
