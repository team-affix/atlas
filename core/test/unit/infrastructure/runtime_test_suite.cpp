...
#include "infrastructure/dbuct_phoenix_fc_runtime.hpp"
...
#include "infrastructure/phoenix_fc_runtime.hpp"
...
inline constexpr size_t kRidgeFgtCapacity = 10000;
inline constexpr double kQuellWorkDecayK = 0.2;
inline constexpr double kQuellWorkDecayJ = 10.0;

enum class runtime_kind {
    basic,
    ridge, ridge_fgt, ridge_fc, phoenix, phoenix_fc, dbuct_phoenix, dbuct_phoenix_fc,
    dbuct_ridge, dbuct_ridge_fc, dbuct_ridge_fgt,
...
struct runtime_ref {
    using variant_t = std::variant<
        basic_runtime*,
        ridge_runtime*, ridge_fgt_runtime*, ridge_fc_runtime*, phoenix_runtime*, phoenix_fc_runtime*,
        dbuct_phoenix_runtime*, dbuct_phoenix_fc_runtime*,
        dbuct_ridge_runtime*, dbuct_ridge_fc_runtime*, dbuct_ridge_fgt_runtime*,
...
struct runtime_session_holder {
    ...
    std::optional<ridge_fgt_runtime> ridge_fgt;
    std::optional<ridge_fc_runtime> ridge_fc;
    std::optional<phoenix_runtime> phoenix;
    std::optional<phoenix_fc_runtime> phoenix_fc;
    std::optional<dbuct_phoenix_runtime> dbuct_phoenix;
    std::optional<dbuct_phoenix_fc_runtime> dbuct_phoenix_fc;
...
runtime_ref& make_runtime_session(
    ...
    switch (kind) {
        ...
        case runtime_kind::ridge:
            ...
            return *holder.ref;
        case runtime_kind::phoenix:
            holder.phoenix.emplace(
                database,
                goals,
                initial_frame_offset,
                max_resolutions,
                seed,
                kRidgeExplorationConstant);
            holder.ref.emplace(runtime_ref::variant_t{&*holder.phoenix});
            return *holder.ref;
        case runtime_kind::phoenix_fc:
            holder.phoenix_fc.emplace(
                database,
                goals,
                initial_frame_offset,
                max_resolutions,
                seed,
                kRidgeExplorationConstant);
            holder.ref.emplace(runtime_ref::variant_t{&*holder.phoenix_fc});
            return *holder.ref;
        case runtime_kind::dbuct_phoenix:
            holder.dbuct_phoenix.emplace(
                database,
                goals,
                initial_frame_offset,
                max_resolutions,
                seed,
                kRidgeExplorationConstant);
            holder.ref.emplace(runtime_ref::variant_t{&*holder.dbuct_phoenix});
            return *holder.ref;
        case runtime_kind::dbuct_phoenix_fc:
            holder.dbuct_phoenix_fc.emplace(
                database,
                goals,
                initial_frame_offset,
                max_resolutions,
                seed,
                kRidgeExplorationConstant);
            holder.ref.emplace(runtime_ref::variant_t{&*holder.dbuct_phoenix_fc});
            return *holder.ref;
        case runtime_kind::ridge_fgt:
            holder.ridge_fgt.emplace(
                database,
                goals,
                initial_frame_offset,
                max_resolutions,
                ...
                kRidgeExplorationConstant,
                kRidgeFgtCapacity);
            holder.ref.emplace(runtime_ref::variant_t{&*holder.ridge_fgt});
            return *holder.ref;
        case runtime_kind::ridge_fc:
            holder.ridge_fc.emplace(
                database,
                goals,
                initial_frame_offset,
                max_resolutions,
        ...
        case runtime_kind::horizon:
            ...
            return *holder.ref;
        case runtime_kind::horizon_fc:
            holder.horizon_fc.emplace(
                database,
                goals,
                initial_frame_offset,
                max_resolutions,
        ...
        case runtime_kind::genius:
            ...
            return *holder.ref;
        case runtime_kind::genius_fc:
            holder.genius_fc.emplace(
                database,
                goals,
                initial_frame_offset,
                max_resolutions,
        ...
        case runtime_kind::dbuct_genius:
            ...
            return *holder.ref;
        case runtime_kind::dbuct_genius_fc:
            holder.dbuct_genius_fc.emplace(
                database,
                goals,
                initial_frame_offset,
                max_resolutions,
                ...
                kRidgeExplorationConstant,
                kRidgeExplorationConstant);
            holder.ref.emplace(runtime_ref::variant_t{&*holder.dbuct_genius_fc});
            return *holder.ref;
    }
...
TEST_P(RuntimeParamTest, HorizonGeniusCgwAfterUnitSolution) {
    if (GetParam() != runtime_kind::horizon
        && GetParam() != runtime_kind::horizon_fc
        && GetParam() != runtime_kind::genius
        && GetParam() != runtime_kind::genius_fc
        && GetParam() != runtime_kind::dbuct_horizon
        && GetParam() != runtime_kind::dbuct_horizon_fc
        && GetParam() != runtime_kind::dbuct_genius
        && GetParam() != runtime_kind::dbuct_genius_fc)
        GTEST_SKIP() << "cgw() is horizon*/genius*/dbuct_horizon*/dbuct_genius* only";
    ...
    } else if (GetParam() == runtime_kind::horizon_fc) {
        ASSERT_TRUE(holder_.horizon_fc.has_value());
        EXPECT_NEAR(holder_.horizon_fc->cgw(), 1.0, 1e-9);
    } else if (GetParam() == runtime_kind::genius) {
        ASSERT_TRUE(holder_.genius.has_value());
        EXPECT_NEAR(holder_.genius->cgw(), 1.0, 1e-9);
    } else if (GetParam() == runtime_kind::genius_fc) {
        ASSERT_TRUE(holder_.genius_fc.has_value());
        EXPECT_NEAR(holder_.genius_fc->cgw(), 1.0, 1e-9);
    } else if (GetParam() == runtime_kind::dbuct_horizon) {
        ASSERT_TRUE(holder_.dbuct_horizon.has_value());
        EXPECT_NEAR(holder_.dbuct_horizon->cgw(), 1.0, 1e-9);
    } else if (GetParam() == runtime_kind::dbuct_horizon_fc) {
        ASSERT_TRUE(holder_.dbuct_horizon_fc.has_value());
        EXPECT_NEAR(holder_.dbuct_horizon_fc->cgw(), 1.0, 1e-9);
    } else if (GetParam() == runtime_kind::dbuct_genius) {
        ASSERT_TRUE(holder_.dbuct_genius.has_value());
        EXPECT_NEAR(holder_.dbuct_genius->cgw(), 1.0, 1e-9);
    } else {
        ASSERT_TRUE(holder_.dbuct_genius_fc.has_value());
        EXPECT_NEAR(holder_.dbuct_genius_fc->cgw(), 1.0, 1e-9);
    }
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(runtime_kind::basic,
                      ...
                      runtime_kind::phoenix, runtime_kind::phoenix_fc,
                      runtime_kind::dbuct_phoenix, runtime_kind::dbuct_phoenix_fc,
    ...
    [](const ::testing::TestParamInfo<runtime_kind>& info) {
        switch (info.param) {
            ...
            case runtime_kind::ridge_fgt:          return "ridge_fgt";
            case runtime_kind::ridge_fc:           return "ridge_fc";
            case runtime_kind::phoenix:            return "phoenix";
            case runtime_kind::phoenix_fc:         return "phoenix_fc";
            case runtime_kind::dbuct_phoenix:      return "dbuct_phoenix";
            case runtime_kind::dbuct_phoenix_fc:   return "dbuct_phoenix_fc";
