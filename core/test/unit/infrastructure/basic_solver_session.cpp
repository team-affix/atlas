...
TEST_F(BasicSolverSessionTest, EnumeratesPeanoLessThanSeven) {
    ...
    basic_solver_session session(database, initial_goals, kInitialVarCount, kPeanoBudget, kSeed);
    enumerate_all_solutions(
        session,
        saved_printer_,
        expected,
...
TEST_F(BasicSolverSessionTest, EnumeratesK3ThreeColorings) {
    ...
    basic_solver_session session(database, initial_goals, kInitialVarCount, kColorBudget, kSeed);
    std::set<solution> expected = {
...
TEST_F(BasicSolverSessionTest, EnumeratesFourVarSatThreeClauses) {
    ...
    basic_solver_session session(database, initial_goals, kInitialVarCount, kSatBudget, kSeed);
    std::set<solution> expected = {
...
TEST_F(BasicSolverSessionTest, EnumeratesCatalanTreesWithFiveNodes) {
    ...
    basic_solver_session session(database, initial_goals, kInitialVarCount, kCatalanBudget, kSeed);
    std::set<solution> visited;
    while (session.next()) {
        if (!session.solved())
            continue;
...
TEST_F(BasicSolverSessionTest, EnumeratesCollatzOneStepPreimagesOfTen) {
    ...
    basic_solver_session session(database, initial_goals, kInitialVarCount, kCollatzBudget, kSeed);
    enumerate_all_solutions(
        session,
        saved_printer_,
...
TEST_F(BasicSolverSessionTest, EnumeratesOrderedCompositionsOfEight) {
    /*
...
TEST_F(BasicSolverSessionTest, EnumeratesBinaryStringsNoConsecutiveOnesLengthTen) {
    ...
    basic_solver_session session(database, initial_goals, kBinaryStringsInitialVarCount, kTierIBudget, kSeed);
    enumerate_all_solutions(
        session,
        saved_printer_,
        expected,
...
TEST_F(BasicSolverSessionTest, EnumeratesStaircasePathsOneOrTwoSummingToTen) {
    /*
