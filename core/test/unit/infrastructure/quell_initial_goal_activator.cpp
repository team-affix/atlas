// quell_initial_goal_activator: delegates to initial_goal_activator, then prices
// the initial goal's work, records it, and adds that work to remaining.
...
struct QuellInitialGoalActivatorTest : public ::testing::Test {
    ...
    test_quell_initial_goal_activator_t activator{
        mock_initial, make_initial_goal_lineage, set_goal_work_value,
        get_goal_work, add_remaining_work};
};

TEST_F(QuellInitialGoalActivatorTest, ActivatesThenSetsWorkAndAddsRemaining) {
    EXPECT_CALL(get_goal_work, get(&gl)).Times(AtLeast(1)).WillRepeatedly(Return(kDecay0));
