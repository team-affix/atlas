// lp_solver differs from solver only in when and how the avoidance is learned:
// learn() takes no argument and must run before tear-down, because tear-down
// clears the decision set learn() reads. These tests pin that ordering.
...
struct MockLearnAvoidance {
    MOCK_METHOD(std::optional<const resolution_lineage*>, learn, (), ());
};
...
TEST_F(LpSolverTest, LearnRunsBeforeTearDown) {
    ...
    EXPECT_CALL(learn, learn()).WillOnce(Return(std::nullopt));
    EXPECT_CALL(tear_down, tear_down());

    drive();
}

TEST_F(LpSolverTest, EliminationIsRoutedAfterTearDown) {
    InSequence seq;
    EXPECT_CALL(set_up, set_up());
    EXPECT_CALL(run, run()).WillOnce(Return(sim_termination::conflicted));
    EXPECT_CALL(decision_count, count()).WillOnce(Return(0));
    EXPECT_CALL(derive, derive_decision_lemma()).WillOnce(Return(empty_lemma));
    EXPECT_CALL(learn, learn()).WillOnce(Return(std::optional{&g0_r0}));
    EXPECT_CALL(tear_down, tear_down());
    EXPECT_CALL(router, route(&g0_r0)).WillOnce(Return(elimination_result::eliminated));
