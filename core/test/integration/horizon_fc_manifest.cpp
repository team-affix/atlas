// Integration: horizon_fc_manifest — fewer-candidate scores + horizon CGW reward.
...
#include "infrastructure/horizon_fc_manifest.hpp"
...
class HorizonFcManifestIntegrationTest : public ::testing::Test {
protected:
    ...
    horizon_fc_manifest make_manifest(size_t max_resolutions = kMaxResolutions) {
        return horizon_fc_manifest{
            database,
            initial_goals,
            kInitialVarCount,
            max_resolutions,
...
TEST_F(HorizonFcManifestIntegrationTest, InitialActivateSetsFewerCandidateScore) {
    ...
    horizon_fc_manifest manifest = make_manifest();
...
TEST_F(HorizonFcManifestIntegrationTest, TearDownClearsRpScores) {
    horizon_fc_manifest manifest = make_manifest();
    manifest.horizon_set_up_sim_.set_up();
