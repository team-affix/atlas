// Integration: dbuct_horizon_manifest — delayed-backtracking (horizon_dbuct) solver.
//
// The horizon_dbuct solver camps deep in the search tree and backtracks lazily
...
// horizon solver finds (soundness + completeness), remain correct across lazy
// backsteps (state, bindings, and learned avoidances restored to each resume
// frontier), and actually camp rather than restart.
//
// Most correctness checks are DIFFERENTIAL against horizon_runtime: for a given
...
class DbuctHorizonManifestIntegrationTest : public ::testing::Test {
protected:
    ...
    dbuct_horizon_manifest make_manifest(uint32_t initial_frame_offset = 0,
                                 size_t max_resolutions = kMaxResolutions,
                                 size_t grant_interval = kGrantInterval) {
        return dbuct_horizon_manifest{
            database, initial_goals, initial_frame_offset, max_resolutions, kSeed,
            kExplorationConstant, grant_interval};
    }

    dbuct_horizon_runtime make_dbuct(uint32_t initial_var_count,
                             size_t max_resolutions = kMaxResolutions,
                             size_t grant_interval = kGrantInterval) {
        return dbuct_horizon_runtime{database, initial_goals, initial_var_count,
                             max_resolutions, kSeed, kExplorationConstant,
                             grant_interval};
    }

    horizon_runtime make_horizon(uint32_t initial_var_count,
                             size_t max_resolutions = kMaxResolutions) {
        return horizon_runtime{database, initial_goals, initial_var_count,
                             max_resolutions, kSeed, kExplorationConstant};
    }
...
TEST_F(DbuctHorizonManifestIntegrationTest, WiringConstructsWithEmptyProblem) {
    EXPECT_NO_THROW(make_manifest());
}

TEST_F(DbuctHorizonManifestIntegrationTest, WiringCampingStackDistinctFromEliminators) {
    dbuct_horizon_manifest m = make_manifest();
    EXPECT_NE(static_cast<void*>(&m.cdcl_), static_cast<void*>(&m.dbuct_joint_));
}

// ── Scenario 2: vacuous / trivial refutation contract ────────────────────────

TEST_F(DbuctHorizonManifestIntegrationTest, VacuousSolvedThenRefutesOnEmptyProblem) {
    dbuct_horizon_runtime rt = make_dbuct(0);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctHorizonManifestIntegrationTest, RefutesWhenInitialGoalHasNoCandidates) {
...
TEST_F(DbuctHorizonManifestIntegrationTest, SingleUnitSolutionMakesNoDecision) {
    initial_goals.push(fun("f"));
...
TEST_F(DbuctHorizonManifestIntegrationTest, ClauseDerivedUnitSolutionChainsSubgoals) {
    initial_goals.push(fun("f"));
...
TEST_F(DbuctHorizonManifestIntegrationTest, GroundHeadBindingsAreCorrect) {
    ...
    dbuct_horizon_runtime rt = make_dbuct(2);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctHorizonManifestIntegrationTest, ClauseBodyBindingsFlowThroughSubgoals) {
...
TEST_F(DbuctHorizonManifestIntegrationTest, SolverCampsBelowRootAcrossEpisodes) {
    ...
    dbuct_horizon_manifest m = make_manifest(/*initial_frame_offset=*/1);
...
TEST_F(DbuctHorizonManifestIntegrationTest, ParitySingleBindingSolution) {
    const expr* abc = fun("abc");
    const expr* _123 = fun("123");
    database.push(rule{fun("f", {abc, _123}), {}});
    initial_goals.push(fun("f", {pool.make_var(0), pool.make_var(1)}));
    ...
    horizon_runtime r = make_horizon(2);
    std::set<solution> expected = collect_solutions(
        r, [&] { return get(r); }, kTickCap);

    dbuct_horizon_runtime d = make_dbuct(2);
...
TEST_F(DbuctHorizonManifestIntegrationTest, ParityMultipleGroundFacts) {
    ...
    horizon_runtime r = make_horizon(1);
    std::set<solution> expected = collect_solutions(r, [&] { return get(r); }, kTickCap);

    dbuct_horizon_runtime d = make_dbuct(1);
...
TEST_F(DbuctHorizonManifestIntegrationTest, ParityTwoGoalsCrossProduct) {
...
TEST_F(DbuctHorizonManifestIntegrationTest, ParityConflictDrivenPruning) {
...
TEST_F(DbuctHorizonManifestIntegrationTest, BindingsRemainSoundAcrossEnumeration) {
...
TEST_F(DbuctHorizonManifestIntegrationTest, ImportedSolutionSurvivesFurtherTicks) {
    const expr* abc = fun("abc");
...
TEST_F(DbuctHorizonManifestIntegrationTest, TightResolutionBudgetTerminatesGracefully) {
    ...
    dbuct_horizon_runtime d = make_dbuct(1, /*max_resolutions=*/4);
...
TEST_F(DbuctHorizonManifestIntegrationTest, RecursionEnumeratesOnlyWellFormedNats) {
    ...
    dbuct_horizon_runtime d = make_dbuct(1, /*max_resolutions=*/8);
    std::set<solution> got = collect_solutions(
        d, [&] { return solution{pool.import(d.normalize({pool.make_var(0), 0}))}; },
        256);
...
TEST_F(DbuctHorizonManifestIntegrationTest, GrantIntervalDoesNotChangeSolutionSet) {
    ...
    dbuct_horizon_runtime small = make_dbuct(1, kMaxResolutions, /*grant_interval=*/1);
    std::set<solution> got_small = collect_solutions(small, [&] { return get(small); }, kTickCap);

    dbuct_horizon_runtime large = make_dbuct(1, kMaxResolutions, /*grant_interval=*/1024);
...
TEST_F(DbuctHorizonManifestIntegrationTest, RuntimeFacadeDepthsAfterBranchingDecision) {
    ...
    dbuct_horizon_runtime rt = make_dbuct(1);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctHorizonManifestIntegrationTest, DecisionCountConsistentAcrossManyTicks) {
    ...
    dbuct_horizon_manifest m = make_manifest(/*initial_frame_offset=*/2);
    auto sm = m.solver_.solve();
    for (size_t t = 0; t < 64; ++t) {
        sm.resume();
        if (!sm.has_yield()) break;
...
TEST_F(DbuctHorizonManifestIntegrationTest, CgwIsOneAfterUnitSolution) {
    database.push(rule{fun("f"), {}});
    initial_goals.push(fun("f"));
    dbuct_horizon_runtime rt = make_dbuct(0);
    ASSERT_TRUE(rt.next());
    ASSERT_TRUE(rt.solved());
    EXPECT_NEAR(rt.cgw(), 1.0, 1e-9);
}

TEST_F(DbuctHorizonManifestIntegrationTest, HorizonHubPopRestoresWeightsAndCgw) {
    dbuct_horizon_manifest m = make_manifest();
    goal_lineage gl{nullptr, 0};
    m.horizon_hub_.push_solver_frame();
    m.goal_weights_.set(&gl, 0.5);
    m.cumulative_grounded_weight_.accumulate(0.5);
    EXPECT_DOUBLE_EQ(m.goal_weights_.get(&gl), 0.5);
    EXPECT_DOUBLE_EQ(m.cumulative_grounded_weight_.get(), 0.5);

    auto sm = m.horizon_hub_.pop_solver_frame();
    while (!sm.done())
        sm.resume();

    EXPECT_THROW(m.goal_weights_.get(&gl), std::out_of_range);
