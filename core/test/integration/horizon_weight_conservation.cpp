...
TEST_F(HorizonWeightConservationIntegrationTest, FailedSubgoalActivationBanksNoWeight) {
    // A non-fact expansion whose candidate activation fails. horizon_resolver only
    // banks weight for facts, and the real resolver stops before its deactivation
    // tail, so the parent must keep its full weight and nothing may reach
    // cumulative_grounded_weight -- banked weight is never given back, so a leak
    // here is a permanent over-reward.
    //
    // Note this state is NOT weight-conserving: the subgoals activated before the
    // failure are already on the frontier carrying their split of the parent's
    // weight while the parent itself is still live, so the frontier transiently
    // OVER-counts. That is by design -- a failed activation aborts the sim
    // (tear_down_sim clears every store) or, under dbuct, is undone by the frame
    // pop. What must never happen is weight going missing, since that
    // under-rewards permanently; assert the direction rather than equality.
    horizon_initial_goal_activator_->activate_initial_goal(0);
    srt_active_goals_.flush_srt_goal_batch();
    const goal_lineage* root = make_initial_goal_lineage_->make(0);
    const resolution_lineage* expand_rl = lineage_pool_.make_resolution_lineage(root, 0);
    ...
    ON_CALL(activate_goal_candidates, activate_goal_candidates).WillByDefault(Return(false));

    EXPECT_FALSE(horizon_resolver_->resolve(expand_rl));

    EXPECT_DOUBLE_EQ(cumulative_grounded_weight_.get(), 0.0);
    EXPECT_NEAR(goal_weights_.get(root), kTotalWeight, kWeightEpsilon);
    EXPECT_TRUE(srt_active_goals_.is_active_goal(root));
    EXPECT_GE(sum_active_weights(goal_weights_, srt_active_goals_)
                  + cumulative_grounded_weight_.get(),
              kTotalWeight - kWeightEpsilon);
}

TEST_F(HorizonWeightConservationIntegrationTest, FailedFactResolveStillBanksOnlyItsOwnWeight) {
    // A fact banks its weight BEFORE delegating, so a downstream failure must not
    // double-bank or bank a sibling's share on retry.
    horizon_initial_goal_activator_->activate_initial_goal(0);
    srt_active_goals_.flush_srt_goal_batch();
    const goal_lineage* root = make_initial_goal_lineage_->make(0);
    const resolution_lineage* expand_rl = lineage_pool_.make_resolution_lineage(root, 0);
    ...
    const goal_lineage* g_goal = lineage_pool_.make_goal_lineage(expand_rl, 0);
    const resolution_lineage* g_rl = lineage_pool_.make_resolution_lineage(g_goal, 1);
    candidate_frame_offsets_.set(g_rl, 0u);

    ON_CALL(activate_goal_candidates, activate_goal_candidates).WillByDefault(Return(false));

    // A fact has an empty body, so the real resolver's subgoal loop never runs and
    // the resolve still succeeds; exactly this goal's half is banked.
    ASSERT_TRUE(horizon_resolver_->resolve(g_rl));
    EXPECT_NEAR(cumulative_grounded_weight_.get(), kTotalWeight / 2.0, kWeightEpsilon);
    expect_conserved();
}
