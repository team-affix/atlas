// Integration: dbuct_phoenix_manifest — delayed-backtracking (phoenix_dbuct) solver.
//
// The phoenix_dbuct solver camps deep in the search tree and backtracks lazily
...
// phoenix solver finds (soundness + completeness), remain correct across lazy
// backsteps (state, bindings, and learned avoidances restored to each resume
// frontier), and actually camp rather than restart.
//
// Most correctness checks are DIFFERENTIAL against phoenix_runtime: for a given
...
#include "infrastructure/dbuct_phoenix_runtime.hpp"
...
class DbuctPhoenixManifestIntegrationTest : public ::testing::Test {
protected:
    ...
    dbuct_phoenix_manifest make_manifest(uint32_t initial_frame_offset = 0,
                                 size_t max_resolutions = kMaxResolutions,
                                 double grant_k = kGrantK) {
        return dbuct_phoenix_manifest{
            database, initial_goals, initial_frame_offset, max_resolutions, kSeed,
            kExplorationConstant, grant_k};
    }

    dbuct_phoenix_runtime make_dbuct(uint32_t initial_var_count,
                             size_t max_resolutions = kMaxResolutions,
                             double grant_k = kGrantK) {
        return dbuct_phoenix_runtime{database, initial_goals, initial_var_count,
                             max_resolutions, kSeed, kExplorationConstant,
                             grant_k};
    }

    phoenix_runtime make_phoenix(uint32_t initial_var_count,
                             size_t max_resolutions = kMaxResolutions) {
        return phoenix_runtime{database, initial_goals, initial_var_count,
                             max_resolutions, kSeed, kExplorationConstant};
    }
...
TEST_F(DbuctPhoenixManifestIntegrationTest, WiringConstructsWithEmptyProblem) {
    EXPECT_NO_THROW(make_manifest());
}

TEST_F(DbuctPhoenixManifestIntegrationTest, WiringCampingStackDistinctFromEliminators) {
    dbuct_phoenix_manifest m = make_manifest();
    EXPECT_NE(static_cast<void*>(&m.cdcl_), static_cast<void*>(&m.dbuct_joint_));
}

// ── Scenario 2: vacuous / trivial refutation contract ────────────────────────

TEST_F(DbuctPhoenixManifestIntegrationTest, VacuousSolvedThenRefutesOnEmptyProblem) {
    dbuct_phoenix_runtime rt = make_dbuct(0);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctPhoenixManifestIntegrationTest, RefutesWhenInitialGoalHasNoCandidates) {
...
TEST_F(DbuctPhoenixManifestIntegrationTest, SingleUnitSolutionMakesNoDecision) {
    initial_goals.push(fun("f"));
...
TEST_F(DbuctPhoenixManifestIntegrationTest, ClauseDerivedUnitSolutionChainsSubgoals) {
    initial_goals.push(fun("f"));
...
TEST_F(DbuctPhoenixManifestIntegrationTest, GroundHeadBindingsAreCorrect) {
    ...
    dbuct_phoenix_runtime rt = make_dbuct(2);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctPhoenixManifestIntegrationTest, ClauseBodyBindingsFlowThroughSubgoals) {
...
TEST_F(DbuctPhoenixManifestIntegrationTest, SolverCampsBelowRootAcrossEpisodes) {
    ...
    dbuct_phoenix_manifest m = make_manifest(/*initial_frame_offset=*/1);
...
TEST_F(DbuctPhoenixManifestIntegrationTest, ParitySingleBindingSolution) {
    const expr* abc = fun("abc");
    const expr* _123 = fun("123");
    database.push(rule{fun("f", {abc, _123}), {}, 0});
    initial_goals.push(fun("f", {pool.make_var(0), pool.make_var(1)}));
    ...
    phoenix_runtime r = make_phoenix(2);
    std::set<solution> expected = collect_solutions(
        r, [&] { return get(r); }, kTickCap);

    dbuct_phoenix_runtime d = make_dbuct(2);
...
TEST_F(DbuctPhoenixManifestIntegrationTest, ParityMultipleGroundFacts) {
    ...
    phoenix_runtime r = make_phoenix(1);
    std::set<solution> expected = collect_solutions(r, [&] { return get(r); }, kTickCap);

    dbuct_phoenix_runtime d = make_dbuct(1);
...
TEST_F(DbuctPhoenixManifestIntegrationTest, ParityTwoGoalsCrossProduct) {
...
TEST_F(DbuctPhoenixManifestIntegrationTest, ParityConflictDrivenPruning) {
...
TEST_F(DbuctPhoenixManifestIntegrationTest, BindingsRemainSoundAcrossEnumeration) {
...
TEST_F(DbuctPhoenixManifestIntegrationTest, ImportedSolutionSurvivesFurtherTicks) {
    const expr* abc = fun("abc");
...
TEST_F(DbuctPhoenixManifestIntegrationTest, TightResolutionBudgetTerminatesGracefully) {
    ...
    dbuct_phoenix_runtime d = make_dbuct(1, /*max_resolutions=*/4);
...
TEST_F(DbuctPhoenixManifestIntegrationTest, RecursionEnumeratesOnlyWellFormedNats) {
    ...
    dbuct_phoenix_runtime d = make_dbuct(1, /*max_resolutions=*/8);
    std::set<solution> got = collect_solutions(
        d, [&] { return solution{pool.import(d.normalize({pool.make_var(0), 0}))}; },
        256);
...
TEST_F(DbuctPhoenixManifestIntegrationTest, GrantIntervalDoesNotChangeSolutionSet) {
    ...
    dbuct_phoenix_runtime small = make_dbuct(1, kMaxResolutions, /*grant_k=*/0.0);
    std::set<solution> got_small = collect_solutions(small, [&] { return get(small); }, kTickCap);

    dbuct_phoenix_runtime large = make_dbuct(1, kMaxResolutions, /*grant_k=*/0.0024);
...
TEST_F(DbuctPhoenixManifestIntegrationTest, RuntimeFacadeDepthsAfterBranchingDecision) {
    ...
    dbuct_phoenix_runtime rt = make_dbuct(1);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctPhoenixManifestIntegrationTest, DecisionCountConsistentAcrossManyTicks) {
    ...
    dbuct_phoenix_manifest m = make_manifest(/*initial_frame_offset=*/2);
    auto sm = m.solver_.solve();
    for (size_t t = 0; t < 64; ++t) {
        sm.resume();
        if (!sm.has_yield()) break;
