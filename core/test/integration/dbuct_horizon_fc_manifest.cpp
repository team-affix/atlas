// Integration: dbuct_horizon_fc_manifest — FC scores + frame undo (camping).
...
class DbuctHorizonFcManifestIntegrationTest : public ::testing::Test {
protected:
    ...
    dbuct_horizon_fc_manifest make_manifest(size_t max_resolutions = kMaxResolutions) {
        return dbuct_horizon_fc_manifest{
            database,
            initial_goals,
            kInitialVarCount,
            max_resolutions,
...
TEST_F(DbuctHorizonFcManifestIntegrationTest, InitialActivateSetsFewerCandidateScore) {
...
TEST_F(DbuctHorizonFcManifestIntegrationTest, FramePopUndoesRpScores) {
    dbuct_horizon_fc_manifest manifest = make_manifest();
...
TEST_F(DbuctHorizonFcManifestIntegrationTest, WiringConstructsWithEmptyProblem) {
