// Integration: dbuct_ridge_bt_manifest — delayed-backtracking ridge with binary-tree CDCL.
//
// The dbuct-ridge-bt solver camps deep in the search tree and backtracks lazily
...
// ridge-bt solver finds (soundness + completeness), remain correct across lazy
// backsteps (state, bindings, and learned avoidances restored to each resume
// frontier), and actually camp rather than restart.
//
// Most correctness checks are DIFFERENTIAL against ridge_bt_runtime: for a given
...
#include "infrastructure/dbuct_ridge_bt_runtime.hpp"
...
protected:
    ...
    dbuct_ridge_bt_manifest make_manifest(uint32_t initial_frame_offset = 0,
                                 size_t max_resolutions = kMaxResolutions,
                                 double grant_k = kGrantK) {
        return dbuct_ridge_bt_manifest{
            database, initial_goals, initial_frame_offset, max_resolutions, kSeed,
            kExplorationConstant, grant_k};
    }

    dbuct_ridge_bt_runtime make_dbuct(uint32_t initial_var_count,
                             size_t max_resolutions = kMaxResolutions,
                             double grant_k = kGrantK) {
        return dbuct_ridge_bt_runtime{database, initial_goals, initial_var_count,
                             max_resolutions, kSeed, kExplorationConstant,
                             grant_k};
    }

    ridge_bt_runtime make_ridge_bt(uint32_t initial_var_count,
                             size_t max_resolutions = kMaxResolutions) {
        return ridge_bt_runtime{database, initial_goals, initial_var_count,
                             max_resolutions, kSeed, kExplorationConstant};
    }
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, WiringConstructsWithEmptyProblem) {
    EXPECT_NO_THROW(make_manifest());
}

TEST_F(DbuctRidgeBtManifestIntegrationTest, WiringCampingStackDistinctFromEliminators) {
    dbuct_ridge_bt_manifest m = make_manifest();
    EXPECT_NE(static_cast<void*>(&m.cdcl_), static_cast<void*>(&m.dbuct_joint_));
}

// ── Scenario 2: vacuous / trivial refutation contract ────────────────────────

TEST_F(DbuctRidgeBtManifestIntegrationTest, VacuousSolvedThenRefutesOnEmptyProblem) {
    dbuct_ridge_bt_runtime rt = make_dbuct(0);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, RefutesWhenInitialGoalHasNoCandidates) {
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, SingleUnitSolutionMakesNoDecision) {
    initial_goals.push(fun("f"));
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, ClauseDerivedUnitSolutionChainsSubgoals) {
    initial_goals.push(fun("f"));
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, GroundHeadBindingsAreCorrect) {
    ...
    dbuct_ridge_bt_runtime rt = make_dbuct(2);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, ClauseBodyBindingsFlowThroughSubgoals) {
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, SolverCampsBelowRootAcrossEpisodes) {
    ...
    dbuct_ridge_bt_manifest m = make_manifest(/*initial_frame_offset=*/1);
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, ParitySingleBindingSolution) {
    const expr* abc = fun("abc");
    const expr* _123 = fun("123");
    database.push(rule{fun("f", {abc, _123}), {}, 0});
    initial_goals.push(fun("f", {pool.make_var(0), pool.make_var(1)}));
    ...
    ridge_bt_runtime r = make_ridge_bt(2);
    std::set<solution> expected = collect_solutions(
        r, [&] { return get(r); }, kTickCap);

    dbuct_ridge_bt_runtime d = make_dbuct(2);
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, ParityMultipleGroundFacts) {
    ...
    ridge_bt_runtime r = make_ridge_bt(1);
    std::set<solution> expected = collect_solutions(r, [&] { return get(r); }, kTickCap);

    dbuct_ridge_bt_runtime d = make_dbuct(1);
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, ParityTwoGoalsCrossProduct) {
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, ParityConflictDrivenPruning) {
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, BindingsRemainSoundAcrossEnumeration) {
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, ImportedSolutionSurvivesFurtherTicks) {
    const expr* abc = fun("abc");
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, TightResolutionBudgetTerminatesGracefully) {
    ...
    dbuct_ridge_bt_runtime d = make_dbuct(1, /*max_resolutions=*/4);
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, RecursionEnumeratesOnlyWellFormedNats) {
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, GrantIntervalDoesNotChangeSolutionSet) {
    ...
    dbuct_ridge_bt_runtime small = make_dbuct(1, kMaxResolutions, /*grant_k=*/0.0);
    std::set<solution> got_small = collect_solutions(small, [&] { return get(small); }, kTickCap);

    dbuct_ridge_bt_runtime large = make_dbuct(1, kMaxResolutions, /*grant_k=*/0.0024);
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, RuntimeFacadeDepthsAfterBranchingDecision) {
    ...
    dbuct_ridge_bt_runtime rt = make_dbuct(1);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctRidgeBtManifestIntegrationTest, DecisionCountConsistentAcrossManyTicks) {
    ...
    dbuct_ridge_bt_manifest m = make_manifest(/*initial_frame_offset=*/2);
    auto sm = m.solver_.solve();
    for (size_t t = 0; t < 64; ++t) {
        sm.resume();
        if (!sm.has_yield()) break;
