// Integration: phoenix_fc_manifest — fewer-candidate score wiring on phoenix (ridge reward + dual chooser).
...
class PhoenixFcManifestIntegrationTest : public ::testing::Test {
protected:
    ...
    phoenix_fc_manifest make_manifest(size_t max_resolutions = kMaxResolutions) {
        return phoenix_fc_manifest{
            database,
            initial_goals,
            kInitialVarCount,
            max_resolutions,
...
TEST_F(PhoenixFcManifestIntegrationTest, InitialActivateSetsFewerCandidateScore) {
    /*
    ...
    phoenix_fc_manifest manifest = make_manifest();
...
TEST_F(PhoenixFcManifestIntegrationTest, TearDownClearsRpScores) {
    /*
