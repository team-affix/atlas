// Integration: dbuct_genius_manifest — delayed-backtracking (genius_dbuct) solver.
//
// The genius_dbuct solver camps deep in the search tree and backtracks lazily
...
// genius solver finds (soundness + completeness), remain correct across lazy
// backsteps (state, bindings, and learned avoidances restored to each resume
// frontier), and actually camp rather than restart.
//
// Most correctness checks are DIFFERENTIAL against genius_runtime: for a given
...
class DbuctGeniusManifestIntegrationTest : public ::testing::Test {
protected:
...
TEST_F(DbuctGeniusManifestIntegrationTest, WiringConstructsWithEmptyProblem) {
    EXPECT_NO_THROW(make_manifest());
}

TEST_F(DbuctGeniusManifestIntegrationTest, WiringCampingStackDistinctFromEliminators) {
    dbuct_genius_manifest m = make_manifest();
    EXPECT_NE(static_cast<void*>(&m.cdcl_), static_cast<void*>(&m.dbuct_joint_));
}

// ── Scenario 2: vacuous / trivial refutation contract ────────────────────────

TEST_F(DbuctGeniusManifestIntegrationTest, VacuousSolvedThenRefutesOnEmptyProblem) {
    dbuct_genius_runtime rt = make_dbuct(0);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctGeniusManifestIntegrationTest, RefutesWhenInitialGoalHasNoCandidates) {
...
TEST_F(DbuctGeniusManifestIntegrationTest, SingleUnitSolutionMakesNoDecision) {
    initial_goals.push(fun("f"));
...
TEST_F(DbuctGeniusManifestIntegrationTest, ClauseDerivedUnitSolutionChainsSubgoals) {
    initial_goals.push(fun("f"));
...
TEST_F(DbuctGeniusManifestIntegrationTest, GroundHeadBindingsAreCorrect) {
    ...
    dbuct_genius_runtime rt = make_dbuct(2);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctGeniusManifestIntegrationTest, ClauseBodyBindingsFlowThroughSubgoals) {
...
TEST_F(DbuctGeniusManifestIntegrationTest, SolverCampsBelowRootAcrossEpisodes) {
    ...
    dbuct_genius_manifest m = make_manifest(/*initial_frame_offset=*/1);
...
TEST_F(DbuctGeniusManifestIntegrationTest, ParitySingleBindingSolution) {
    const expr* abc = fun("abc");
    const expr* _123 = fun("123");
    database.push(rule{fun("f", {abc, _123}), {}});
    initial_goals.push(fun("f", {pool.make_var(0), pool.make_var(1)}));
...
TEST_F(DbuctGeniusManifestIntegrationTest, ParityMultipleGroundFacts) {
...
TEST_F(DbuctGeniusManifestIntegrationTest, ParityTwoGoalsCrossProduct) {
...
TEST_F(DbuctGeniusManifestIntegrationTest, ParityConflictDrivenPruning) {
...
TEST_F(DbuctGeniusManifestIntegrationTest, BindingsRemainSoundAcrossEnumeration) {
...
TEST_F(DbuctGeniusManifestIntegrationTest, ImportedSolutionSurvivesFurtherTicks) {
    const expr* abc = fun("abc");
...
TEST_F(DbuctGeniusManifestIntegrationTest, TightResolutionBudgetTerminatesGracefully) {
    ...
    dbuct_genius_runtime d = make_dbuct(1, /*max_resolutions=*/4);
...
TEST_F(DbuctGeniusManifestIntegrationTest, RecursionEnumeratesOnlyWellFormedNats) {
    ...
    dbuct_genius_runtime d = make_dbuct(1, /*max_resolutions=*/8);
    std::set<solution> got = collect_solutions(
        d, [&] { return solution{pool.import(d.normalize({pool.make_var(0), 0}))}; },
        256);
...
TEST_F(DbuctGeniusManifestIntegrationTest, RuntimeFacadeDepthsAfterBranchingDecision) {
    ...
    dbuct_genius_runtime rt = make_dbuct(1);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctGeniusManifestIntegrationTest, DecisionCountConsistentAcrossManyTicks) {
    ...
    dbuct_genius_manifest m = make_manifest(/*initial_frame_offset=*/2);
    auto sm = m.solver_.solve();
    for (size_t t = 0; t < 64; ++t) {
        sm.resume();
        if (!sm.has_yield()) break;
...
TEST_F(DbuctGeniusManifestIntegrationTest, CgwIsOneAfterUnitSolution) {
    database.push(rule{fun("f"), {}});
    initial_goals.push(fun("f"));
    dbuct_genius_runtime rt = make_dbuct(0);
    ASSERT_TRUE(rt.next());
...
TEST_F(DbuctGeniusManifestIntegrationTest, GeniusHubPopRestoresWeightsAndCgw) {
    dbuct_genius_manifest m = make_manifest();
