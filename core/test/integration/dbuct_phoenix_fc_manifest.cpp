// Integration: dbuct_phoenix_fc_manifest — fewer-candidate scores + frame undo.
...
class DbuctPhoenixFcManifestIntegrationTest : public ::testing::Test {
protected:
    ...
    dbuct_phoenix_fc_manifest make_manifest(size_t max_resolutions = kMaxResolutions) {
        return dbuct_phoenix_fc_manifest{
            database,
            initial_goals,
            kInitialVarCount,
            max_resolutions,
...
TEST_F(DbuctPhoenixFcManifestIntegrationTest, InitialActivateSetsFewerCandidateScore) {
    /*
    ...
    dbuct_phoenix_fc_manifest manifest = make_manifest();
...
TEST_F(DbuctPhoenixFcManifestIntegrationTest, FramePopUndoesRpScores) {
    /*
...
TEST_F(DbuctPhoenixFcManifestIntegrationTest, WiringConstructsWithEmptyProblem) {
