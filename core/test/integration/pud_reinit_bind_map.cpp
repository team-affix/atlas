...
struct PudReinitBindMapIntegrationTest : public ::testing::Test {
    PudReinitBindMapIntegrationTest()
        : unify_head_(om_, parent_, added_unifications_, fpa_, fpa_, glob_, exprs_, exprs_) {}
...
TEST_F(PudReinitBindMapIntegrationTest, PathReplayIsVisibleInQueryNodeInterval) {
    ...
    pud_candidate_search_context env{
    ...
    om_interval out_env = env.interval;
    EXPECT_TRUE(unify_head_.unify_callee(env, child, touched_reps, out_env));
    const std::optional<framed_expr> found = fpa_.query(out_env.open, 0);
...
TEST_F(PudReinitBindMapIntegrationTest, UnifyCalleeBindingsVisibleInEnvInterval) {
    ...
    std::vector<uint32_t> touched_reps;
    om_interval out_env = env.interval;
    EXPECT_TRUE(unify_head_.unify_callee(env, axiom, touched_reps, out_env));
    const std::optional<framed_expr> found = fpa_.query(out_env.open, 0);
    ASSERT_TRUE(found.has_value());
    EXPECT_EQ(found->skeleton, head);
}
...
TEST_F(PudReinitBindMapIntegrationTest, EnsureFailedMidPathDoesNotBindLaterNode) {
    ...
    pud_candidate_search_context env{
        ...
        {},
        {}};
    unify_head_.unify_head(env, leaf);
    EXPECT_FALSE(fpa_.query(env.interval.open, 1).has_value());
}
