// Integration: dbuct_quell_manifest — delayed-backtracking (quell_dbuct) solver.
//
// The quell_dbuct solver camps deep in the search tree and backtracks lazily
...
// quell solver finds (soundness + completeness), remain correct across lazy
// backsteps (state, bindings, and learned avoidances restored to each resume
// frontier), and actually camp rather than restart.
//
// Most correctness checks are DIFFERENTIAL against quell_runtime: for a given
...
class DbuctQuellManifestIntegrationTest : public ::testing::Test {
protected:
    ...
    dbuct_quell_manifest make_manifest(uint32_t initial_frame_offset = 0,
                                 size_t max_resolutions = kMaxResolutions,
                                 size_t grant_interval = kGrantInterval) {
        return dbuct_quell_manifest{
            database, initial_goals, initial_frame_offset, max_resolutions, kSeed,
            kExplorationConstant, kWorkDecayK, kWorkDecayJ, grant_interval};
    }

    dbuct_quell_runtime make_dbuct(uint32_t initial_var_count,
                             size_t max_resolutions = kMaxResolutions,
                             size_t grant_interval = kGrantInterval) {
        return dbuct_quell_runtime{database, initial_goals, initial_var_count,
                             max_resolutions, kSeed, kExplorationConstant,
                             kWorkDecayK, kWorkDecayJ, grant_interval};
    }

    quell_runtime make_quell(uint32_t initial_var_count,
                             size_t max_resolutions = kMaxResolutions) {
        return quell_runtime{database, initial_goals, initial_var_count,
                             max_resolutions, kSeed, kExplorationConstant,
                             kWorkDecayK, kWorkDecayJ};
    }
...
TEST_F(DbuctQuellManifestIntegrationTest, WiringConstructsWithEmptyProblem) {
    EXPECT_NO_THROW(make_manifest());
}

TEST_F(DbuctQuellManifestIntegrationTest, WiringCampingStackDistinctFromEliminators) {
    dbuct_quell_manifest m = make_manifest();
    EXPECT_NE(static_cast<void*>(&m.cdcl_), static_cast<void*>(&m.dbuct_joint_));
}

// ── Scenario 2: vacuous / trivial refutation contract ────────────────────────

TEST_F(DbuctQuellManifestIntegrationTest, VacuousSolvedThenRefutesOnEmptyProblem) {
...
TEST_F(DbuctQuellManifestIntegrationTest, RefutesWhenInitialGoalHasNoCandidates) {
...
TEST_F(DbuctQuellManifestIntegrationTest, ClauseBodyBindingsFlowThroughSubgoals) {
...
TEST_F(DbuctQuellManifestIntegrationTest, SolverCampsBelowRootAcrossEpisodes) {
...
TEST_F(DbuctQuellManifestIntegrationTest, ParityMultipleGroundFacts) {
...
TEST_F(DbuctQuellManifestIntegrationTest, ParityTwoGoalsCrossProduct) {
...
TEST_F(DbuctQuellManifestIntegrationTest, ParityConflictDrivenPruning) {
...
TEST_F(DbuctQuellManifestIntegrationTest, BindingsRemainSoundAcrossEnumeration) {
...
TEST_F(DbuctQuellManifestIntegrationTest, TightResolutionBudgetTerminatesGracefully) {
...
TEST_F(DbuctQuellManifestIntegrationTest, RecursionEnumeratesOnlyWellFormedNats) {
    ...
    dbuct_quell_runtime d = make_dbuct(1, /*max_resolutions=*/8);
    std::set<solution> got = collect_solutions(
        d, [&] { return solution{pool.import(d.normalize({pool.make_var(0), 0}))}; },
        256);
...
TEST_F(DbuctQuellManifestIntegrationTest, GrantIntervalDoesNotChangeSolutionSet) {
    ...
    dbuct_quell_runtime small = make_dbuct(1, kMaxResolutions, /*grant_interval=*/1);
    std::set<solution> got_small = collect_solutions(small, [&] { return get(small); }, kTickCap);

    dbuct_quell_runtime large = make_dbuct(1, kMaxResolutions, /*grant_interval=*/1024);
...
TEST_F(DbuctQuellManifestIntegrationTest, RuntimeFacadeDepthsAfterBranchingDecision) {
...
TEST_F(DbuctQuellManifestIntegrationTest, DecisionCountConsistentAcrossManyTicks) {
    ...
    dbuct_quell_manifest m = make_manifest(/*initial_frame_offset=*/2);
    auto sm = m.solver_.solve();
    for (size_t t = 0; t < 64; ++t) {
        sm.resume();
        if (!sm.has_yield()) break;
...
TEST_F(DbuctQuellManifestIntegrationTest, RemainingWorkIsZeroAfterUnitSolution) {
    ...
    ASSERT_TRUE(rt.next());
    ASSERT_TRUE(rt.solved());
    EXPECT_NEAR(rt.remaining_work(), 0.0, 1e-9);
}

TEST_F(DbuctQuellManifestIntegrationTest, QuellHubPopRestoresDepthsWorkValuesAndRemainingWork) {
    dbuct_quell_manifest m = make_manifest();
