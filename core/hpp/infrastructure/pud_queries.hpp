...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
bool pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::rule_id_less::operator()(
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::pud_queries(
        IGN& get_node,
        IRI& root_interval,
...
pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::root_contexts() {
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::replace_owned(
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::watch(
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::watch_context(
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::watch_query(pud_query& query) {
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::unwatch_query(pud_query* query) {
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::resume_query(pud_query& query) {
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::mark_dirty(const pud_rule_id* leaf) {
    dirty_leaves_.insert(leaf);
}

template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::commit_queries(
        const pud_rule_id* leaf, std::vector<pud_query> queries) {
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::install(const pud_rule_id* leaf) {
    const pud_db_node& node = get_node_.get_node(leaf);
    const uint32_t frame_offset = node.lvc;
    const om_interval leaf_interval = root_interval_.root_interval(leaf);
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::boot() {
    for (const pud_rule_id* axiom : axioms_)
        install(axiom);
}
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::ensure_booted() {
    if (booted_)
        return;
    booted_ = true;
    boot();
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::adopt_axiom(
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::fork_child(
        const pud_rule_id* child,
        const std::vector<pud_query>& leftover_templates) {
    const pud_db_node& stored_child = get_node_.get_node(child);
    const uint32_t child_lvc = stored_child.lvc;
    const om_interval child_interval = root_interval_.root_interval(child);
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::clear(const pud_rule_id* leaf) {
...
pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::get(const pud_rule_id* leaf) {
    ensure_booted();
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::resume_dead_witness(
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::invalidate_leaf(
...
pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::take_dirty_leaves() {
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
std::vector<const pud_rule_id*>
pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::live_cursors(pud_query& query) {
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
std::vector<const pud_rule_id*>
pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::live_callees(
...
template<typename IGN, typename IRI, typename IACI, typename IR, typename IDQE,
         typename IRCS, typename IRWS>
void pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::replace_unfolded(
...
pud_queries<IGN, IRI, IACI, IR, IDQE, IRCS, IRWS>::take_forced_unfolds() {
    ensure_booted();
