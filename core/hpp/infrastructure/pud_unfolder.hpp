...
private:
    ...
    const pud_rule_id* materialize_child(const pud_rule_id* leaf,
                                         size_t body_goal_idx,
    ...
    void install_child_queries(const pud_rule_id* child,
...
pud_unfolder<IGN, IBQ, IUC, IN, IMV, IAI, ILC, IACI, IGLQ, IRLQ, ICLQ, IR, IRCS,
             IOR, IOL, ITP, IIL, IW, IUQ>::find_live_callees(pud_query& query) {
...
const pud_rule_id*
pud_unfolder<IGN, IBQ, IUC, IN, IMV, IAI, ILC, IACI, IGLQ, IRLQ, ICLQ, IR, IRCS,
             IOR, IOL, ITP, IIL, IW, IUQ>::materialize_child(
        const pud_rule_id* leaf,
        size_t body_goal_idx,
        const pud_rule_id* callee,
        uint32_t parent_lvc,
...
template<typename IGN, typename IBQ, typename IUC, typename IN, typename IMV,
         ...
         typename ITP, typename IIL, typename IW, typename IUQ>
void pud_unfolder<IGN, IBQ, IUC, IN, IMV, IAI, ILC, IACI, IGLQ, IRLQ, ICLQ, IR, IRCS,
                  IOR, IOL, ITP, IIL, IW, IUQ>::install_child_queries(
    ...
    for (const pud_query& leftover : leftover_templates) {
        ...
        forked.interval = allocate_child_interval_.allocate_child_of(
            stored_child.interval);
...
pud_unfolder<IGN, IBQ, IUC, IN, IMV, IAI, ILC, IACI, IGLQ, IRLQ, ICLQ, IR, IRCS,
    ...
    for (const pud_rule_id* callee : callees)
        children.push_back(materialize_child(
            leaf, body_goal_idx, callee, parent_lvc, parent_interval));
    ...
    for (const pud_rule_id* child : children)
        install_child_queries(child, leftover_templates);
