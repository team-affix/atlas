#ifndef RIDGE_LP_MANIFEST_HPP
#define RIDGE_LP_MANIFEST_HPP
...
// ridge with lazy-propagation CDCL: the avoidance store is decision-frame
// indexed and propagates new avoidances one edge at a time. Differs from
// ridge_manifest only in cdcl_t, the extra lp_decision_recorder_t that drives
// descent, and lp_solver_t in place of solver_t.
struct ridge_lp_manifest {
    ...
    using cdcl_t  = lp_cdcl_elimination_generator;
    ...
    lp_decision_recorder_t           lp_decision_recorder_;
    run_sim_t                      run_sim_;
    solver_t                      solver_;
