...
struct dbuct_runtime {
    ...
    bool next() { return driver_.next(); }
    bool solved() const { return driver_.solved(); }
    const expr* normalize(framed_expr fe) { return normalizer_.normalize(fe); }
    size_t resolution_depth() const { return manifest_.resolution_memory_.get_resolution_count(); }
    size_t decision_depth() const { return manifest_.decision_memory_.count(); }
    lemma derive_decision_lemma() const { return manifest_.decision_memory_.derive_decision_lemma(); }
    lemma derive_resolution_lemma() const { return manifest_.resolution_memory_.derive_resolution_lemma(); }
