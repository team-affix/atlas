...
TEST_F(ImportGoalsFromStringTest, MultipleGoalsShareVar) {
    ...
    EXPECT_EQ(goals.get(0), pool->make_functor(functor_map.at("p"), {x}));
    EXPECT_EQ(goals.get(1), pool->make_functor(functor_map.at("q"), {x}));
...
TEST_F(ImportGoalsFromStringTest, ComplexGoals) {
    ...
    EXPECT_EQ(goals.get(0), pool->make_functor(functor_map.at("reach"), {x, y}));
    EXPECT_EQ(goals.get(1), pool->make_functor(functor_map.at("next"), {y, z}));
    EXPECT_EQ(goals.get(2), pool->make_functor(functor_map.at("base"), {z}));
}
