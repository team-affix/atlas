...
TEST_F(DatabaseVisitorTest, VisitDatabase) {
    db database;
    database_visitor dv{*pool, *pool, *var_seq, database};
    ...
    EXPECT_EQ(r0->head, pool->make("base", {x0}));
    EXPECT_THAT(r0->body, IsEmpty());

    const expr* x1 = pool->make(1);
    const rule* r1 = rule_at(database, rule_id{1});
    EXPECT_EQ(r1->head, pool->make("step", {x1}));
    EXPECT_THAT(r1->body, SizeIs(1));
    EXPECT_EQ(r1->body[0], pool->make("base", {x1}));
